      ...
      <Card className="glass-panel">
        ...
        <CardContent className="grid gap-3 md:grid-cols-2">
          {data.incomes.map((income) => (
            <div key={income.id} className="rounded-xl border bg-background/50 p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-medium">{income.source}</p>
                  <p className="text-sm text-muted-foreground">{income.category?.name ?? "Income"} · {income.date.toLocaleDateString()}</p>
                </div>
                <Badge variant="secondary">{income.frequency.toLowerCase()}</Badge>
              </div>
              <p className="mt-4 text-2xl font-semibold text-emerald-400">{currency(toNumber(income.amount))}</p>
            </div>
          ))}
        </CardContent>
      </Card>
